...
CREATE INDEX IF NOT EXISTS idx_comanda_estado_hora ON comandas (estado, hora_captura);

-- Índice para Cierre de Cuenta
-- Filtra rápidamente las comandas activas por mesa
CREATE INDEX IF NOT EXISTS idx_comanda_mesa_estado ON comandas (mesa_id, estado);

-- Índice para el detalle de comandas
-- Acelera el proceso CerrarCuentaMesa que suma subtotales de productos 'activos'
CREATE INDEX IF NOT EXISTS idx_detalle_comanda_estado ON detalle_comandas (comanda_id, estado);

-- Índices Transaccionales y de Reportes Analíticos (HTMX)
-- Acelera los arqueos y reportes financieros
CREATE INDEX IF NOT EXISTS idx_tickets_corte ON tickets (corte_id);
CREATE INDEX IF NOT EXISTS idx_tickets_fecha ON tickets (fecha_cierre);

-- Acelera los reportes del Reloj Checador
CREATE INDEX IF NOT EXISTS idx_asistencias_fecha ON asistencias_personal (timestamp);

-- Índice para filtrado en el Log Viewer
CREATE INDEX IF NOT EXISTS idx_syslogs_level_time ON sys_logs (level, timestamp);
