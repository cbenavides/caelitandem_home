...
if [[ -z "$LAESH_DOMAIN" ]]; then
    ...
    echo "     BASE=https://${LAESH_IP} bash ~/staging/setup/bds/laesh/bash/verify/03_test_deploy.sh"
    exit 0
fi
...
echo "     BASE=https://${LAESH_DOMAIN} bash ~/staging/setup/bds/laesh/bash/verify/03_test_deploy.sh"
