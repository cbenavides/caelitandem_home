...
*(Nota Legal: Independientemente de la opción elegida, toda la documentación contractual se ejecuta utilizando un Contrato Base Modular, el cual protege las garantías, previene retrasos y garantiza el cumplimiento fiscal para ambas partes).*
...
### Opción 3: Bloc Digital (Aplicación Web de Solicitudes)
...
| Módulo / Interfaz | Opción 1 | Opción 2 | Opción 3 | Opción 4 | Ruta de Acceso |
| :--- | :---: | :---: | :---: | :---: | :--- |
...
*   **Funcionalidades Fuera de Alcance:** A fin de mantener un ecosistema eficiente y sin ambigüedades, queda expresamente fuera de todas las opciones planteadas (4) el desarrollo de:
    *   Aplicaciones móviles nativas instalables desde tiendas (App Store/Google Play). Todo el ecosistema opera de manera 100% web.
    *   Chatbots automáticos para responder dudas médicas, de precios o dar servicio al cliente vía WhatsApp.
    *   Punto de Venta (Terminales), pasarelas de pago con tarjeta en línea o integraciones de control de caja.
    *   Módulos financieros avanzados: Facturación Electrónica (CFDI), sistemas de nómina, o calculadoras de comisiones para pagar a médicos.
    *   **Asesoría Legal (INAI):** La redacción jurídica del *Aviso de Privacidad* y el cumplimiento de la Ley de Protección de Datos Personales respecto al envío de diagnósticos por WhatsApp, es responsabilidad absoluta de "EL CLIENTE" y sus asesores (el desarrollador solo publica el texto que se le entregue).
    *   **Cuentas de Correo Corporativo:** El proyecto no incluye el alojamiento ni creación de buzones de correo para empleados (ej. recepcion@laesh.mx). El sistema utilizará un motor invisible (SMTP) exclusivo para alertas automáticas internas.
...
1. **Póliza de Soporte a Producción (Mantenimiento):** $8,000 MXN anuales. Incluye bolsa de 12 horas mensuales para cambios o resolución rápida de incidentes.
2. **Administración de Servidor / Hosting:** $4,000 MXN anuales. (Exclusivo para la Opción 2, 3 y 4. No necesario para la Opción 1). Cubre la mano de obra técnica (Respaldos, parches de seguridad), *el hardware se paga directo al proveedor*.
...
1. **Riesgo de Aprobación en Meta (WhatsApp Cloud API):** Meta exige la validación formal de la empresa para habilitar el uso oficial de su API, lo cual puede retrasar la salida a producción.
   * *Mitigación:* "EL CLIENTE" proveerá los documentos de verificación (Constancia Fiscal y Comprobante de Domicilio) durante los primeros días del proyecto para que Meta apruebe la cuenta anticipadamente, evitando cuellos de botella en el despliegue.
2. **Riesgo de Extracción de Texto (PDFs del Laboratorio):** Existe la posibilidad de que los equipos médicos de la clínica generen PDFs planos (tipo imagen escaneada), haciendo imposible la lectura automática del folio.
   * *Mitigación:* Se ejecutará una Prueba de Concepto (PoC) técnica en etapas tempranas. De resultar inviable, se activará la regla de contingencia operativa acordada: el personal renombrará manualmente el archivo con el `#Folio` respectivo.
3. **Riesgo de Caídas de Infraestructura de Terceros:** Al depender de proveedores externos (Hostinger y Meta Platforms), el sistema está expuesto a interrupciones por mantenimiento o caídas globales de dichos servicios.
   * *Mitigación:* Contratación sugerida de la *Póliza de Administración de Servidor* para garantizar copias de seguridad continuas y una reactivación ágil (SLA de 2 horas) una vez que el proveedor restablezca el servicio.
